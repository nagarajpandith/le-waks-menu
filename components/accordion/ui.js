        ...
        dishes.map((dish) => {
          return (
            <AnimatePresence key={dish.dishId}>
              <motion.div
                key={dish.dishId}
                className="flex flex-row justify-between items-center bg-white md:pl-10 font-semibold text-black w-full md:w-3/4 h-auto rounded-md p-4 shadow-md mb-3 dark:bg-gray-500"
                ...
                <div className="inline-flex flex-col gap-y-3 w-[75%] md:w-[50%]">
                  <Link href={`/dish/` + dish.dishId}>
                    <a className="text-gray-700 dark:text-white text-sm md:text-lg">
                      {locale === 'en' ? dish.dish : dish.ar}
                    </a>
                  </Link>
                  {/* <a className='text-gray-500 font-light'>{dish.desc}</a> */}
                  <a className="text-yellow-400 dark:text-yellow-300 text-sm md:text-lg">
                    {locale === 'en' ? `${dish.price} SAR` : dish.ar_pr}
                  </a>
                  {title === 'Burgers' ? (
                    <a className="text-gray-700 dark:text-white text-xs md:text-md font-normal">
                      <ReadMoreReact
                        ...
                        max={40}
                        readMoreText={
                          <a className="cursor-pointer text-bold text-yellow-400 dark:text-yellow-300">
                            Read More
                          </a>
                        }
                        text={
                  ...
                  <div className="inline-flex flex-row">
                    <a className="text-gray-400 font-light text-xs dark:text-white md:text-lg">
                      {dish.cal} Calories
                    </a>
          ...
          {dish.spcl? <div><Image src="/star-badge.png" width={25} height={25} alt="star" /></div> : null}
                ...
                <Link href={'/dish/' + dish.dishId}>
                  <Image
                    className="rounded-lg w-[25%] md:w-[50%]"
                    src={dish.image}
                    width={150}
                    height={150}
                    alt="dish-image"
